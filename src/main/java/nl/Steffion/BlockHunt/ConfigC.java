...
public enum ConfigC {
    ...
    wandName("%A&l" + BlockHunt.pdfFile.getName() + "%N's selection wand", W.config),
    ...
    shop_blockChooserv1Description(new String[]{"%NUse this item before the arena starts.",
    ...
    sign_SHOP(new String[]{
            "%H[" + BlockHunt.pdfFile.getName() + "%H]", "&4SHOP", "&8Right-Click", "&8To shop."}, W.config),
    sign_WAITING(new String[]{
            "%H[" + BlockHunt.pdfFile.getName() + "%H]", "%A%arenaname%", "%A%players%%N/%A%maxplayers%", "&8Waiting..."}, W.config),
    sign_STARTING(new String[]{
            "%H[" + BlockHunt.pdfFile.getName() + "%H]", "%A%arenaname%", "%A%players%%N/%A%maxplayers%", "&2Start: %A%timeleft%"}, W.config),
    sign_INGAME(new String[]{
    ...
    log_enabledPlugin("%TAG%N%name%&a&k + %N%version% is now Enabled. Made by %A%autors%%N.", W.messages), log_disabledPlugin("%TAG%N%name%&c&k - %N%version% is now Disabled. Made by %A%autors%%N.", W.messages),
    ...
    help_join("%NJoins a " + BlockHunt.pdfFile.getName() + " game.", W.messages),
    help_leave("%NLeave a " + BlockHunt.pdfFile.getName() + " game.", W.messages),
    help_list("%NShows a list of available arenas.", W.messages),
    help_shop("%NOpens the " + BlockHunt.pdfFile.getName() + " shop.", W.messages),
    ...
    normal_ingameHiderDied("%TAG%NHider %A%playername%%N was killed by %A%killer%%N!", W.messages),
    normal_ingameHidersLeft("%NHider(s) left: %A%left%%N", W.messages),
    normal_ingameSeekerDied("%TAG%NSeeker %A%playername%%N died and will respawn in %A%secs%%N seconds!", W.messages),
    ...
    normal_shopChoosenBlock("%TAG%NYou've choosen to be a(n) '%A%block%%N' block!", W.messages),
    normal_shopChoosenSeeker("%TAG%NYou've choosen to be a %Aseeker%N!", W.messages),
    normal_shopChoosenHiders("%TAG%NYou've choosen to be a %Ahider%N!", W.messages),
    normal_ingameBlocksLeft("%TAG%NRemaining blocks: %A%1%%N", W.messages),
    warning_lobbyNeedAtleast("%TAG%WYou need atleast %A%1%%W player(s) to start the game!", W.messages),
    ...
    error_commandNotEnabled("%TAG%EThis command has been disabled!", W.messages),
    error_commandNotFound("%TAG%ECouldn't find the command. Try %A/" + BlockHunt.pdfFile.getName()
            + " help %Efor more info.", W.messages),
    ...
    error_joinAlreadyJoined(
            "%TAG%EYou've already joined an arena!", W.messages),
    ...
    error_joinInventoryNotEmpty("%TAG%EYour inventory should be empty before joining!", W.messages),
    ...
    error_createNotSameWorld("%TAG%EMake your selection points in the same world!", W.messages),
