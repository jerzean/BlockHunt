...
public class OnPlayerInteractEvent implements Listener {
    ...
    public void onPlayerInteractEvent(PlayerInteractEvent event) {
        ...
        if (event.getAction() == Action.LEFT_CLICK_BLOCK || event.getAction() == Action.LEFT_CLICK_BLOCK && event.getClickedBlock() != null) {
            for (Arena arena : MemoryStorage.arenaList) {
                if (arena.seekers.contains(player)) {
                    for (Player pl : arena.playersInArena) {
                        if (MemoryStorage.hiddenLoc.get(pl) != null) {
                            ...
                            if (pLoc != null && moveLocBlock.getX() == pLoc.getX() && moveLocBlock.getY() == pLoc.getY() && moveLocBlock.getZ() == pLoc.getZ()) {
